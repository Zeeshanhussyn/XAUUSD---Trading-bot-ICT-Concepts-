...
> Does a PDH/PDL or Asia High/Low liquidity-sweep → HTF-bias → 5m MSS/CHOCH+displacement → FVG
...
| HTF bias | Daily = primary, 4H = confirmation, **FLEXIBLE** rule: Daily clear + 4H neutral/transition → trade allowed only if 5m MSS/CHOCH+displacement confirms Daily direction; 4H clearly opposing → blocked | **default** — STRICT (Daily+4H must both align) tested in WP10 |
...
| Displacement | Variant A: candle body ≥ 1.5 × average body of the preceding 20 candles (5m, causal, current candle excluded) | **default** — Variant B (range ≥ 1.5 × ATR(14), 5m, causal) tested in WP10. Lookbacks (20-bar average, ATR 14) confirmed by user |
...
| Pending entry validity | Maximum 5 × 5m candles (25 minutes) | **default** — current-session-only and 10×5m (50 min) variants tested in WP10. Cancelled immediately on opposite structure break, sweep-extreme invalidation, or session end (hard rule) |
| Stop loss | Variant B: beyond MSS/CHOCH invalidation swing | **default** — Variant A (beyond sweep extreme) tested in WP10 |
| SL buffer | 0.1 × ATR(14), 5m, causal | confirmed by user |
...
| Re-entry after loss | Allowed only with fresh liquidity event + fresh 5m MSS/CHOCH + fresh displacement, tagged `re-entry` | fixed (from brief) |
